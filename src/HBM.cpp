...
void HBM::init_speed()
{
	...
	// Mz: tREF1B = 32ms / ((no. of banks) × (no. of rows)). 32ms refresh interval assumed. Jedec manula for HBM2
	// Though below calculation does not match

    const static int REFI1B_TABLE[int(Speed::MAX)][int(Org::MAX)] = {
        {64, 128, 256},
        {128, 256, 512}	// just doubled since prev tCK = 2ns now tCK - 1ns
    };
